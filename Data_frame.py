...
for tahun, total in data_sampah_tahunan.items():
...
for (kota, tahun), total in data_sampah_kota_tahunan.items():
    print(f"{kota} - Tahun {tahun}: {total} ton")

df_sampah_tahun_tertentu = pd.DataFrame({'Tahun': [tahun_tertentu], 'Total Produksi Sampah': [total_tahun]})
df_tahunan = pd.DataFrame(list(data_sampah_tahunan.items()), columns=['Tahun', 'Total Produksi Sampah'])
df_kota_tahunan = pd.DataFrame([(kota, tahun, total) for (kota, tahun), total in data_sampah_kota_tahunan.items()],columns=['nama_kabupaten_kota', 'tahun', 'jumlah_produksi_sampah'])
